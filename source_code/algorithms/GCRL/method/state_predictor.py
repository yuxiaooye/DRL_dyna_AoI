...
class StatePredictor(nn.Module):
    ...
    def forward(self, state, action, detach=False):
        ...
        # else:
        #     next_robot_state = self.compute_next_state(state[0], action)  # 代码中并未对无人机的下一时刻状态进行预测（也确实没必要）
        # # predict
        # next_human_states = self.human_motion_predictor(state_embedding)[:, self.tmp_config.env.robot_num:,:]
        #
        # next_observation = [next_robot_state, next_human_states]  # 人

        next_state_pred = self.mlp(
