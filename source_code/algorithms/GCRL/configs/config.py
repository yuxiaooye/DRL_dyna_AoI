...
class BaseEnvConfig(object):
    ...
    env.energy_factor = 3
    env.robot_num = 2  # 多了要用多进程
    if env.robot_num > 15:
        env.rollout_num = env.robot_num * (env.robot_num - 1)  # 1 2 6 12 15
    else:
        env.rollout_num = 1
    env.robot_human_states = True  # False for freshMCS/DRQN
    ...
    env.nlat = 120
    env.human_num = 59
    env.dataset_dir = 'algorithms/GCRL/envs/crowd_sim/dataset/purdue/59 users.csv'
    ...
    # San Francisco datasets, SEE YOU NEXT TIME
...
class BaseTrainConfig(object):
    ...
    train.num_episodes = 800  # 500
    train.warmup_episodes = 100  # 100, exploration
    train.evaluate_episodes = 1  # 10? 1?
    train.sample_episodes = 1

    # number of episodes sampled in one training episode
    train.target_update_interval = 30
    train.evaluation_interval = 100
    ...
    def __init__(self, debug=False):
        if debug:
            ...
            self.train.num_episodes = 1
            self.train.checkpoint_interval = 1
