...
class YyxAgentBase():
    ...
    def load_nets(self, dir_name, iter=0, best=False):
        ...
        self.actors.load_state_dict(torch.load(dir_name + '/Models/' + prefix + '_actor.pt', map_location=self.device))
        if self.input_args.algo.startswith('G2ANet'):
            self.g2a_embed_hard_net.load_state_dict(torch.load(dir_name + '/Models/' + prefix + '_g2aAtt.pt', map_location=self.device))
