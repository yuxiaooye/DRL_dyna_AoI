...
def render_HTML(output_dir, tag='train', draw_collect_range=False, traj_filename='eps_best.npz'):
    ...
    rm = Roadmap(dataset, poi_num=122)  # TODO DASAP
    ...
    for index, traj in enumerate(trajs.trajectories):
        ...
        if index<uav_num:
            ...
            TimestampedGeoJson(  # 这里解注释了一个try except
                ...
                period="PT15S",
                duration="PT15S",  # 不写这句时，所有历史轨迹都保留；写了这句代表每个点显示多久
                add_last_point=True,
                transition_time=200,  # The duration in ms of a transition from between timestamps.
                max_speed=0.2,
                loop=True,
    ...
    # save
    save_file = os.path.join(output_dir, f'vis_{tag}-222.html')  # TODO DASAP
